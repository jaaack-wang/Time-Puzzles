# adapted from: https://github.com/RUC-NLPIR/WebThinker/blob/main/scripts/search/bing_search.py
import os
import json
import requests
...
def extract_text_from_url(url, use_jina=False, jina_api_key=None, 
                          snippet: Optional[str] = None, keep_links=False):
    """
...
def fetch_page_content(urls, max_workers=32, use_jina=False, 
                       jina_api_key=None, snippets: Optional[dict] = None, 
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            executor.submit(extract_text_from_url, url, use_jina, jina_api_key, 
                            snippets.get(url) if snippets else None, keep_links): url
            for url in urls
        }
...
def google_serper_search(query: str, 
    ...
    # Using 'json=' automatically sets Content-Type and dumps the dict
    payload = {"q": query, "num": num_search_results} 
    ...
    while retry_count < max_retries:
        try:
            ...
            # Check for 4xx errors (Client errors) immediately and DO NOT retry
            if 400 <= response.status_code < 500:
                print(f"Client Error ({response.status_code}): {response.reason}. Not retrying.")
                return {}

            response.raise_for_status() # Will still raise for 5xx errors
            return response.json()

        except (Timeout, HTTPError, RequestException) as e:
            # If it's an HTTPError, we know it's a 5xx (Server error) because we filtered 4xx above.
            # If it's a Timeout or ConnectionError, we also want to retry.
            
            retry_count += 1
            if retry_count == max_retries:
                print(f"API failed after {max_retries} retries. Final error: {e}")
                return {}
